...
cone_combinations(Top_flavors, Bottom_flavors) when is_list(Top_flavors) == false -> fail;
...
cone_combinations(Flavor_list)->
	[{Flavor1, Flavor2} || Flavor1 <- Flavor_list, Flavor2 <- Flavor_list]. 

% TODO: Complete this function so it passes all of its unit tests
most_popular_combinations(Count, _List) when Count < 0 ->
	fail;
most_popular_combinations(Count, _List) when Count > length(_List) ->
	fail;
most_popular_combinations(Count, _List)->
	{Keepers,Rest} = lists:split(Count,_List),
	Keepers.
