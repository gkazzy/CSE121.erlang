...
run(Friend_list) ->
    NewList = receive
        {Pid, {add, Name}} when is_list(Name)->
            Pid ! received,
            Name++Friend_list,
			Friend_list;
		{Pid, {add, Name}}->
			...
			[Name]++Friend_list,
			Friend_list;
		{Pid, {remove, Name}} when is_list(Name)->
			Pid ! received,
			Friend_list--Name,
			Friend_list;
		{Pid, {remove, Name}}->
		...
		{Pid, {has_friend, Friend}} ->
            ...
            Friend_list;
        {Pid, {has_friends, FriendsList}} ->
            Result = lists:all(fun(Friend) -> lists:member(Friend, Friend_list) end, FriendsList),
            Pid ! Result,
            Friend_list;
        {Pid, _Other} ->
